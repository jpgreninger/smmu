...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let translation_faults = smmu.faults().filter(|f| f.fault_type() == FaultType::Translation).count();
    println!("  Translation faults: {}", translation_faults);

    // Filter faults by address range
    let high_addr_faults = smmu.faults().filter(|f| f.address().as_u64() > 0x20000).count();
    ...
    let sum: u32 = smmu.streams().map(|id| id.as_u32()).fold(0, |acc, id| acc + id);
