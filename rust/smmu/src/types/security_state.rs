...
impl SecurityState {
    ...
    pub const fn can_access(self, target: Self) -> bool {
        match (self, target) {
            // Same state can always access itself
            (Self::Secure, Self::Secure) => true,
            (Self::NonSecure, Self::NonSecure) => true,
            (Self::Realm, Self::Realm) => true,

            // Secure can access NonSecure (downgrade)
            (Self::Secure, Self::NonSecure) => true,

            // All other cross-domain access is forbidden
            _ => false,
        }
    }
