...
//   Default is 32 (max SIDSIZE per ARM §6.3.4 IDR1; §6.3.25 effective = MIN(LOG2SIZE,SIDSIZE)).
//
...
// ---------------------------------------------------------------------------
// BUG-R2-CPP-1: reset() must restore strtabLog2Size_ to 32 (default) and cr2_ to 0
// Default is 32 (max SIDSIZE per ARM §6.3.4 IDR1; §6.3.25 effective = MIN(LOG2SIZE,SIDSIZE)).
// ---------------------------------------------------------------------------
...
TEST(BugR2Cpp1, ResetRestoresStrtabLog2Size) {
    ...
    EXPECT_EQ(smmu.getStrtabLog2Size(), 32u)
        << "reset() must restore strtabLog2Size_ to 32 (max SIDSIZE per ARM §6.3.4 IDR1)";
}
...
TEST(BugR2Cpp1, AfterResetHighStreamIDAccepted) {
    ...
    // After reset, strtabLog2Size_ must be 32 (max SIDSIZE per ARM §6.3.4 IDR1).
    EXPECT_EQ(smmu.getStrtabLog2Size(), 32u);
    ...
    // primary assertion) and that StreamID 16 is within the 2^32 range
...
TEST(BugR2Cpp1, DoubleResetRestoresDefaults) {
    ...
    // First reset.
    smmu.reset();
    EXPECT_EQ(smmu.getStrtabLog2Size(), 32u);  // default is 32 (max SIDSIZE per §6.3.4)
    EXPECT_EQ(smmu.getCR2(), 0u);
    // Set again.
    ...
    smmu.reset();
    EXPECT_EQ(smmu.getStrtabLog2Size(), 32u)
        << "second reset() must also restore strtabLog2Size_ to 32 (max SIDSIZE per ARM §6.3.4)";
    EXPECT_EQ(smmu.getCR2(), 0u)
