...
## ✅ **PRODUCTION RELEASE v1.7.0 (C++ + Rust)** - 100% ARM IHI0070G.b Conformance ✅
...
**Latest Update (April 11, 2026)**: v1.7.0 — §13.1.5 MemAttr combining fixed (two-stage S1+S2 attribute merge); §13.2 GBPA bypass Device/NC→OSH shareability enforcement (BUG-13.2-A); §13.6.3 AtsTranslated+ATSCHK=1+EATS=0b10 runs stage-2-only (BUG-13.6.3-A); §13.7 INSTCFG/PRIVCFG overrides applied before stage-2 on split-stage ATS (BUG-13.7-A). C++ tests: 185/185 passing, Rust tests: 212/212 passing, zero clippy warnings.
