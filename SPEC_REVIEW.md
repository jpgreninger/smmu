...
**Overall Conformance**: C++ ~82% | Rust ~91% (software model scope)
_(Baseline was C++ ~68% | Rust ~76% on 2026-02-18; updated after 39 fixes and 2026-02-21 QA re-review)_
...
### FINDING-NEW-11 ❌ — C_BAD_SUBSTREAMID Not Generated for Stage-2-Only / Bypass with Non-Zero PASID
...
Both implementations accept non-zero PASIDs on stage-2-only and bypass streams without faulting. `EventType::C_BAD_SUBSTREAMID` (0x08) / `EventType::CBadSubstreamid` are defined in both enums but are never generated by any code path.

**Evidence**:
- **C++** (`cpp/src/smmu/smmu.cpp`): `performTwoStageTranslation()` stage-2-only branch calls `performStage2OnlyTranslation(streamID, pasid, ...)` without checking `pasid != 0`. Bypass branch returns identity PA with no PASID check.
- **Rust** (`rust/smmu/src/stream_context/mod.rs`): `translate_stage2_only()` and `translate_bypass()` perform no PASID check before proceeding.
- No call to generate `C_BAD_SUBSTREAMID` exists anywhere in either codebase.

**Recommendation**: In the stage-2-only and bypass translation paths of both implementations, check `pasid != 0`. If true, abort with `C_BAD_SUBSTREAMID` (0x08) event. Add `BadSubstreamId` to the `FaultType` enum in both implementations.

---

### FINDING-NEW-12 ❌ — CMD_CFGI_CD and CMD_CFGI_CD_ALL Missing from C++
**Spec**: §4.3.3 (CMD_CFGI_CD, opcode 0x05), §4.3.4 (CMD_CFGI_CD_ALL, opcode 0x06)
**Affected**: C++ only

FINDING-H-03 was marked ✅ but was only fixed in Rust. The C++ `CommandType` enum has no `CFGI_CD = 0x05` or `CFGI_CD_ALL = 0x06` entries. Any software that submits these opcodes to the C++ command queue falls through to the `default:` arm, which incorrectly sets `GERROR_CMDQ_ERR` and generates a spurious `C_BAD_STE` event — active misimplementation.

**Evidence** (`cpp/include/smmu/types.h`, `CommandType` enum): opcodes jump from `CFGI_ALL = 0x04` directly to `TLBI_NH_ALL = 0x10`. No `CFGI_CD` or `CFGI_CD_ALL` present. No handler case in `cpp/src/smmu/smmu.cpp:processCommand()`.

**Recommendation**: Add `CFGI_CD = 0x05` and `CFGI_CD_ALL = 0x06` to C++ `CommandType`. Add handler cases in `processCommand()` routing to `invalidatePASIDCache(streamID, pasid)` and `invalidateStreamCache(streamID)` respectively.

---

### FINDING-NEW-13 ❌ — Stall Mode Always Generates F_TRANSLATION Event Regardless of Actual Fault Type (C++)
**Spec**: §7.3 (Event records), §3.12.2 (Stall model)
**Affected**: C++ only

When stall mode is active and any translation fault occurs, the C++ `translate()` method hard-codes `EventType::F_TRANSLATION` (0x10) in the stall-queue path. If the underlying fault was actually a permission fault (`SMMUError::PagePermissionViolation`), address-size fault (`SMMUError::InvalidAddress`), or security fault, the event record carries the wrong type. This mis-identifies the fault to the OS fault handler, potentially causing incorrect recovery actions.

**Evidence** (`cpp/src/smmu/smmu.cpp`, line ~250): `generateEvent(EventType::F_TRANSLATION, streamID, pasid, iova, securityState, /*isStall=*/true)` is called unconditionally for all error codes.

**Rust status**: Not affected. Rust routes through `map_translation_error_to_fault_type()` → `map_fault_type_to_event_type()` to derive the correct event type.

**Recommendation**: In the C++ stall path in `translate()`, derive the actual `EventType` from `result.getError()` using the same mapping logic as `handleTranslationFailure()`, then pass the derived type to `generateEvent()`.

---

### FINDING-NEW-14 ❌ — PASIDSecurityStateContextSwitching Test Stale After FINDING-L-06 (C++ test debt)
**Spec**: §3.10 (Security states), §3.9 (PASID)
**Affected**: C++ test suite

`PASIDContextSwitchingTest.PASIDSecurityStateContextSwitching` (`cpp/tests/integration/test_pasid_context_switching.cpp:368`) fails because the test body calls `smmu->configureStream(testStreamID, streamConfig)` on a stream already configured by `SetUp()`. FINDING-L-06 made this return `SMMUError::StreamAlreadyConfigured`, breaking the test. The test was not updated when FINDING-L-06 was implemented.

This is test maintenance debt — the stream reconfiguration protection is correct. The spurious second `configureStream()` call should be removed (or preceded by `removeStream()`) in the test body.
...
40. FINDING-NEW-11 — C_BAD_SUBSTREAMID not generated for stage-2-only / bypass with non-zero PASID (Both) ❌
41. FINDING-NEW-12 — CMD_CFGI_CD / CMD_CFGI_CD_ALL missing from C++ ❌
42. FINDING-NEW-13 — Stall mode hard-codes F_TRANSLATION event regardless of actual fault type (C++) ❌
43. FINDING-NEW-14 — PASIDSecurityStateContextSwitching test stale after FINDING-L-06 (C++ test debt) ❌
