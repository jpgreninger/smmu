...
| Section | Title | C++ | Rust | Bugs | Notes |
|---------|-------|-----|------|------|-------|
| §3.1 | Software interface | ✅ | ⚠️ | BUG-AUDIT-148-CPP | C++ re-audited 2026-05-05: 6 items checked, 3 PASS, 1 PARTIAL, 2 N/A. PASS: 3-interface model (streamMap + 3 queues + register surface); IMPDEF=0 baseline; IMPDEF-extended compatibility. N/A: Secure register-set 🚫 out-of-scope; driver-side IMPDEF=0 rule (driver constraint, not SMMU). BUG-AUDIT-148-CPP OPEN: `submitPageRequest()` and `priQueue` always allocated/accessible regardless of `priSupported_`/IDR0.PRI; PRI queue should be unavailable when PRI not advertised (§3.1 line 1220, smmu.cpp `submitPageRequest` ~line 3994). Previously 2026-04-28 Rust audit: all PASS. |
