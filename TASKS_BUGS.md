...
| Section | Title | C++ | Rust | Bugs | Notes |
|---------|-------|-----|------|------|-------|
| §3.1 | Software interface | ✅ | ✅ | | Re-audited 2026-04-28: NS 3-interface model PASS; PRI queue gated on `pri_supported`/IDR0.PRI/PRIQEN PASS; IMPL DEF (`EventEntry.reason`=0) PASS; Secure register set + Secure queues intentionally 🚫 out of scope (§3.10.2) |
...
| §3.3 | Data structures and translation procedure | ⚠️ | ⚠️ | BUG-AUDIT-134/135/136/137 | Re-audited 2026-04-28. PASS: GBPA bypass/abort (`mod.rs:4731-4787`), stage-not-implemented per-STE rejection (`mod.rs:3103-3134`), AP[1] suppression for El2/El3, El2E2h EL1-like privilege. FAIL: (1) BUG-AUDIT-134: no guard rejects `s1p=false AND s2p=false`; (2) BUG-AUDIT-135: no cross-stream ASID/VMID/StreamWorld TLB-config consistency check; (3) BUG-AUDIT-136: no validation same TTB0/granule for streams sharing ASID/VMID/StreamWorld; (4) BUG-AUDIT-137: no STRW-conditional EPD1=1 enforcement for El2/El3 StreamWorlds. Acceptable gaps: FINDING-333-03 (CacheKey excludes VMID/ASID — TLBI scan correctness preserved); S1Fmt not modeled; Realm/Secure StreamWorlds 🚫 out-of-scope. |
...
| Section | Title | C++ | Rust | Bugs | Notes |
|---------|-------|-----|------|------|-------|
| §3.4 | Address sizes (overview) | ✅ | ⚠️ | NEW-GAP-C, AUDIT-47, BUG-AUDIT-138 | T0SZ range, IPS/PS. C++ `AddressSpace::translatePage()` enforces address-size faults; confirmed by BUG-AUDIT-114/115/123 (Rust-only). C++ TDD: `P3AddrSize.*` in `test_cpp_audit_p3_spotcheck.cpp`. Re-audited 2026-04-29: 12 PASS, 3 FAIL, 7 PARTIAL, 18 N/A. FAIL: BUG-AUDIT-138 — canonical VA sign-extension check absent (§3.4 line ~1605, §3.4.1 lines 1664-1665): non-canonical VAs pass T0SZ magnitude guard at `mod.rs:5102-5212` without verifying VA[63:AddrTop+1]==VA[AddrTop]. BUG-AUDIT-139 resolved N/A: queue/MSI base addresses are VecDeque internal state, not PA emissions — same rationale as S1ContextPtr/STE fetch modeling gaps. VAX/VMSAv8-32/VMSAv9-128 N/A: intentionally unsupported (TTF=0b10 hardcoded). |
| §3.4.1 | Input address size and Virtual Address size | ✅ | ⚠️ | AUDIT-47, BUG-AUDIT-138 | T0SZ [16,39] enforcement; T0SZ magnitude check correct for single-TTBR0 model (EPD1/upper-half not modelled). C++ re-verified 2026-04-12 with `P3AddrSize.AddressSizeFaultOnOversizedAddress`. BUG-AUDIT-138 OPEN (Rust): AddrTop sign-extension validation absent — `mod.rs:5114-5118` strips TBI tag but never validates VA[55:48] sign-extended from VA[47] (TBI=1) or VA[63:48] sign-extended from VA[47] (TBI=0). |
