...
| Section | Title | C++ | Rust | Bugs | Notes |
|---------|-------|-----|------|------|-------|
| §3.1 | Software interface | ✅ | ✅ | BUG-AUDIT-148-CPP(FIXED) | C++ re-audited 2026-05-05: 6 items checked, 4 PASS, 2 N/A, 0 bugs. PASS: 3-interface model (streamMap + 3 queues + register surface); IMPDEF=0 baseline; IMPDEF-extended compatibility; PRI queue gating. N/A: Secure register-set 🚫 out-of-scope; driver-side IMPDEF=0 rule (driver constraint, not SMMU). BUG-AUDIT-148-CPP FIXED: `submitPageRequest()` now has `priSupported_` guard (silent drop — no enqueue, no auto-failure); `processPRIQueue()` guarded; `setPRISupported(false)` clears priQueue + resets PROD/CONS to 0. TDD: `test_bug_audit148_pri_gate.cpp` (4 tests). Previously 2026-04-28 Rust audit: all PASS. |
| §3.2 | Stream numbering | ✅ | ✅ | CONF-GAP, AUDIT-82, BUG-AUDIT-120 | Re-audited 2026-04-28: 10 PASS, 1 N/A (SEC_SID — Secure state 🚫 out-of-scope). StreamID u32 full-range (`stream_id.rs:57`); SIDSIZE=32/SSIDSIZE=20 in IDR1; C_BAD_STREAMID range gate (`mod.rs:4789`); C_BAD_SUBSTREAMID on stage1-disabled+SSV=1 (`mod.rs:5236`); PASID_MAX=0xFFFFF (`pasid.rs:36`); strtab_log2size=32 sentinel correct. 0 bugs found. |
| §3.3 | Data structures and translation procedure | ✅ | ✅ | BUG-AUDIT-134(FIXED) | Re-audited 2026-04-28; re-verified 2026-05-05. PASS: GBPA bypass/abort, stage-not-implemented per-STE rejection, AP[1] suppression, El2E2h EL1-like privilege. BUG-AUDIT-134 FIXED: `set_s1p_supported(false)` and `set_s2p_supported(false)` each refuse to clear the last stage (§3.3 "at least one stage required"). BUG-AUDIT-135/136 reclassified N/A: "multiple StreamID configs sharing ASID/VMID/StreamWorld must maintain same TTB/granule" — spec text says "software must ensure" (programming constraint, not SMMU enforcement; §3.3.3 line 1514-1515). BUG-AUDIT-137 reclassified N/A: "CD.TTB1 is unused in EL2/EL3 regimes" — software constraint on CD programming; SMMU does not enforce EPD1=1 (§3.3.3 line 1517). |
...
| Section | Title | C++ | Rust | Bugs | Notes |
|---------|-------|-----|------|------|-------|
| §3.4 | Address sizes (overview) | ✅ | ✅ | NEW-GAP-C, AUDIT-47, BUG-AUDIT-138(FIXED) | T0SZ range, IPS/PS. C++ `AddressSpace::translatePage()` enforces address-size faults. BUG-AUDIT-138 FIXED (Rust): canonical VA sign-extension check added at `mod.rs` (after magnitude check): VA[AddrTop:N-1] must all be identical; non-canonical VA → F_TRANSLATION. TBI=0: AddrTop=63; TBI=1: AddrTop=55. Test: `test_bug_audit138_canonical_va.rs`. Existing test `gap_c_t0sz_16_iova_within_range_succeeds` updated to use canonical VA (0x0000_7FFF_FFFF_F000 instead of non-canonical 0x0000_FFFF_FFFF_F000). BUG-AUDIT-139 N/A: queue/MSI addresses are VecDeque state. VAX/VMSAv8-32/VMSAv9-128 N/A: unsupported. |
| §3.4.1 | Input address size and Virtual Address size | ✅ | ✅ | AUDIT-47, BUG-AUDIT-138(FIXED) | T0SZ [16,39] enforcement; magnitude check + canonical sign-extension check both implemented (Rust). BUG-AUDIT-138 FIXED: `mod.rs` now validates VA[AddrTop:N-1] identical after the magnitude check block. Two new tests cover TBI=0 and TBI=1 non-canonical VAs. |
| §3.4.2 | Address alignment checks | N/A | N/A | | §3.4.2 states SMMU does NOT check address alignment. No implementation needed. |
| §3.4.3 | Address sizes of SMMU-originated accesses | N/A | ✅ | BUG-AUDIT-114, BUG-AUDIT-115, BUG-AUDIT-123 | BUG-AUDIT-114: stage-1-only PA > OAS silently truncates per §3.4 line 1635 — fixed. BUG-AUDIT-115: CD.TTB0/TTB1 out-of-IPS → C_BAD_CD — fixed. BUG-AUDIT-123: IPS=52/OAS=52 range checks skipped (< 52 guard); changed to <= 52 so 2^52 limit enforced — fixed. 5 new tests, 210/210 pass. Modeling gaps (BUG-AUDIT-116/117/118/119) require PA pointer fields not present in model — documented N/A. C++ not applicable. Re-audited 2026-04-29: S1ContextPtr/L1CD.L2Ptr/STE fetch/VMSPtr/queue-MSI base all N/A (flat model, no hardware fetch). |

### §3.5 Command and Event Queues

| Section | Title | C++ | Rust | Bugs | Notes |
|---------|-------|-----|------|------|-------|
| §3.5.1 | SMMU circular queues | ⚠️ | ✅ | CONF-GAP, BUG-RUST-Q2/Q4, BUG-CPP-1/2 | Re-audited 2026-04-30: 26 PASS, 5 N/A, 0 FAIL. advance_index uses 2^(log2size+1) modulus (wrap bit at position log2size ✓); OVFLG at bit[31] correct; stall events buffered in stall_pending on full queue ✓; enqueue_event only for non-stall events ✓; log2size clamped to 19 ✓; consistent init to 0 before enable ✓; no backwards index movement possible ✓; Secure queue N/A (SECURE_IMPL=0). No new defects. |
| §3.5.2 | Queue entry visibility semantics | ⚠️ | ✅ | 2026-03-21, BUG-CPP-1/RUST-1 | Re-audited 2026-04-30: push_back before eventq_prod.store(Release) at all push sites ✓; get_events() never modifies eventq_cons ✓; eventq_cons only written in consume/advance path ✓; EVENTQ interrupt ordering N/A (software model, no async IRQ). No new defects. |
| §3.5.3 | Event queue behavior | ⚠️ | ✅ | AUDIT-75, BUG-AUDIT-154-CPP | Re-audited 2026-05-07 (C++ audit): BUG-AUDIT-154-CPP FIXED — `processEventQueue()` now drains `stallPending_` after consuming entries (smmu.cpp:3392–3406); stall events are promoted when space is next available per §3.5.3 line 1824. TDD: `test_bug_audit154_stall_drain.cpp`. |
| §3.5.4 | Definition of event record write "Commit" | ⚠️ | ✅ | BUG-2/BUG-5 | Re-audited 2026-04-30: stall buffered in stall_pending without PROD advance ✓; stall drain commits with Release store after push_back ✓; non-stall discard has no PROD advance ✓; all PROD.store use Ordering::Release ✓; no phantom commits ✓. No new defects. |
| §3.5.5 | Event merging | N/A | ✅ | | Re-audited 2026-04-30: merging is optional per §3.5.5. Rust implementation optionally deduplicates events gated on per-stream mev() flag (BUG-RUST-2/CONF-GAP-14) — more than required. !event.stall guard unconditionally prevents stall-event merging ✓. STE.MEV per-stream control ✓. No defects. |
...
| Section | Title | C++ | Rust | Bugs | Notes |
|---------|-------|-----|------|------|-------|
| §3.6 | Structure and queue ownership | N/A | N/A | | Re-audited 2026-04-30: 6 items all N/A. All statements use "Arm expects" guidance directed at system software (OS, hypervisor, secure firmware). No behavioral requirements on SMMU model. Secure/Realm queue structures out of scope. Guest-STE conversion and command-queue shadowing are hypervisor responsibilities. 0 defects. |
| §3.7 | Programming registers | ✅ | ✅ | BUG-AUDIT-73, BUG-AUDIT-124 | Re-audited 2026-04-30: 2 checklist items both N/A (hardware memory-map layout; inapplicable to software model). Behavioral register work recorded here: BUG-AUDIT-73: PRIQEN RES0 when IDR0.PRI==0; BUG-AUDIT-124: CMDQEN/EVENTQEN/PRIQEN RO-while-set guard in set_cr0() via (CR0|CR0ACK) preserve; ATSCHK/SMMUEN write-guards on CR1/CR2/strtab all verified. 0 new defects. |
| §3.8 | Virtualization | N/A | N/A | | Re-audited 2026-05-01: 6 sentences, 0 behavioral requirements. All language is descriptive ("Arm expects," "might provide," "might request"). Stage 2-only mapping behavior covered by §3.3. IMPL DEF extra interfaces explicitly "beyond scope." TASKS_OPERATION.md item marked N/A. 0 defects. |

### §3.9 PCI Express, PASIDs, PRI, and ATS

| Section | Title | C++ | Rust | Bugs | Notes |
|---------|-------|-----|------|------|-------|
| §3.9.1 | ATS Interface | ✅ | ⚠️ | AUDIT-01, NEW-FINDING-1, BUG-AUDIT-125, BUG-AUDIT-128, BUG-AUDIT-158-CPP | Re-audited 2026-05-08 (C++): IDR0.ATS=1 ✓, IDR0.PRI configurable ✓, SMMUEN=0→F_BAD_ATS_TREQ ✓ (BUG-AUDIT-156-CPP fixed). BUG-AUDIT-158-CPP fixed: atsSupported now checks CR0_ATSCHK for eats>=2 (effective EATS==0 when EATS==0b1x+ATSCHK==0). HTTU+ATS flag update PASS. Secure items N/A (SecP=0). NOTE: Prior C++ status overclaimed PASS for SMMUEN=0 F_BAD_ATS_TREQ — BUG-AUDIT-128 fix was Rust-only (mod.rs:4439); C++ was not fixed until BUG-AUDIT-156/158-CPP (2026-05-08). |
| §3.9.1.1 | Handling of addresses in ATS-related transactions | ✅ | ⚠️ | | Re-audited 2026-05-08 (C++): IMPL DEF per spec — truncate or abort on PA overflow, both permitted. No behavioral gap. |
| §3.9.1.2 | Responses to ATS Translation Requests | ✅ | ⚠️ | BUG-AUDIT-126, BUG-AUDIT-128, BUG-AUDIT-155-CPP, BUG-AUDIT-156-CPP, BUG-AUDIT-158-CPP | Re-audited 2026-05-08 (C++): BUG-AUDIT-156-CPP fixed: SMMUEN=0→F_BAD_ATS_TREQ now unconditional (was incorrectly gated on REC_CFG_ATS). BUG-AUDIT-158-CPP fixed: effective EATS==0 for eats>=2+ATSCHK=0 correctly fires F_BAD_ATS_TREQ. BUG-AUDIT-155-CPP fixed: ATS TR faults now suppress event and return Success R==W==0. Config=0b000→silent UR ✓, Config=0b100→F_BAD_ATS_TREQ ✓, RnW=UNKNOWN ✓. Secure items N/A. |
| §3.9.1.3 | Handling of ATS Translated transactions | ✅ | ⚠️ | BUG-AUDIT-125, BUG-AUDIT-127 | Re-audited 2026-05-08 (C++): SMMUEN=0→F_TRANSL_FORBIDDEN ✓, Config=0b000+ATSCHK=1→silent abort ✓, Config=0b100+ATSCHK=1→F_TRANSL_FORBIDDEN ✓, EATS=0+ATSCHK=1→F_TRANSL_FORBIDDEN ✓, SSV=1 fault recording ✓, event priority preserved structurally ✓. PASIDTT/Realm items N/A. Secure items N/A. |
| §3.9.1.4 | ATS Invalidation timeout | ⚠️ | ⚠️ | AUDIT-81, BUG-AUDIT-157-CPP | Re-audited 2026-05-08 (C++): BUG-AUDIT-157-CPP: CERROR_ATC_INV_SYNC (value 3) defined in types.h but not wired in CMD_SYNC handler. executeATCInvalidationCommand() has no failure path — software model always succeeds synchronously. Full fix deferred pending ATC endpoint failure simulation infrastructure. |
| §3.9.1.5 | ATS Invalidation errors | N/A | N/A | | Re-audited 2026-05-01: CMD_ATC_INV always completes without error in software model (no real PCIe endpoint). UR from endpoint is out-of-model behavior. Conformant. |
| §3.9.2 | Changing ATS configuration | N/A | N/A | | Re-audited 2026-05-01: All 7 items are software programming sequence guides (driver ordering constraints). No SMMU hardware enforcement required. EATS=0b10+ATSCHK=0 effective-EATS rule now enforced at runtime (BUG-AUDIT-128 fix covers the behavioral portion). |
...
| Section | Title | C++ | Rust | Bugs | Notes |
|---------|-------|-----|------|------|-------|
| §3.10 | Security States (NS support, NS→NS PA) | ✅ | ✅ | | Re-audited 2026-05-08 (C++): SMMU always supports NS (PASS smmu.h:513); NS stream→NS PA via validateSecurityState() smmu.cpp:6393 (PASS); Secure stream→both PA spaces via nsCfg pathway (PASS). |
| §3.10.1 | StreamID Security state (SEC_SID) | ✅ | ✅ | BUG-AUDIT-159-CPP (N/A) | Re-audited 2026-05-13 (C++): 3 items checked, 1 PASS, 2 N/A, 0 bugs. SECURE_IMPL==0 default NS PASS; SECURE_IMPL==1 branch N/A (conditional on capability not advertised; model is NS-only with SECURE_IMPL implicitly 0; BUG-AUDIT-159-CPP re-triaged N/A); SEC_SID 2-bit RME encoding PASS (types.h:508-517). All checkboxes marked [x]. |
| §3.10.2 | Support for Secure state | ✅ | 🚫 | | Re-audited 2026-05-13 (C++): SSec param PASS (CommandEntry::ssec); RAZ/WI register access N/A (no MMIO); PA routing N/A; Secure queue/table N/A (SMMU_S_IDR1 not implemented; NS-only model); SIF N/A (SMMU_S_CR0 not implemented; SIF=0 default correct); S2AA64 check PASS smmu.cpp:1185; VMID tagging PASS smmu.cpp:764-771. BUG-AUDIT-159-CPP/160-CPP re-triaged N/A. |
| §3.10.2.1 | Secure commands, events, configuration | ✅ | 🚫 | | Re-audited 2026-05-13 (C++): NS cmd→NS streams only PASS (SSec=1 CERROR_ILL); event routing N/A (hardware MMIO ring concern; securityState tagged per entry); SIF N/A (SMMU_S_CR0 absent). BUG-AUDIT-159-CPP/160-CPP re-triaged N/A. |
| §3.10.2.2 | Secure EL2 and stage-2 translation | ⚠️ | 🚫 | | Re-audited 2026-05-08 (C++): SEL2 capability bits N/A; S2AA64 check PASS smmu.cpp:1185; VMID tagging PASS smmu.cpp:764-771; nG bit N/A (descriptor parsing not modeled). |
| §3.10.3 | Support for Realm state | ✅ | 🚫 | BUG-AUDIT-161-CPP (N/A), BUG-AUDIT-162-CPP (N/A) | Re-audited 2026-05-13 (C++): 4 items checked, 0 PASS, 4 N/A, 0 bugs. All requirements gated on RME_IMPL=1; model advertises RME_IMPL=0 (smmu.cpp:5354); no SMMU_R_* registers exist. BUG-AUDIT-161-CPP, BUG-AUDIT-162-CPP re-triaged N/A. |
| §3.10.3.1 | Input NS attribute (Realm default) | ✅ | 🚫 | BUG-AUDIT-162-CPP (N/A) | Re-audited 2026-05-13 (C++): 1 item checked, 0 PASS, 1 N/A, 0 bugs. Realm-default NS attribute rule requires Realm programming interface absent in NS-only model (RME_IMPL=0). BUG-AUDIT-162-CPP re-triaged N/A. |
| §3.10.3.2 | Realm stream disabled | ✅ | 🚫 | | Re-audited 2026-05-08 (C++): Config==0b000 → F_STREAM_DISABLED → abort PASS smmu.cpp:715-718,1099. |
| §3.10.3.3 | Realm stream bypass | ✅ | 🚫 | BUG-AUDIT-163-CPP (N/A) | Re-audited 2026-05-13 (C++): 3 items checked, 2 PASS, 1 N/A, 0 bugs. Bypass fault types PASS; NSCFG output PA derivation N/A — SMMU_R_CR0.SMMUEN does not exist; nsCfgOut dead field; Realm bypass path absent. MECID N/A. BUG-AUDIT-163-CPP re-triaged N/A. |
...
| Section | Title | C++ | Rust | Bugs | Notes |
|---------|-------|-----|------|------|-------|
| §3.17 | TLB tagging, VMIDs, ASIDs overview | ✅ | ✅ | BUG-QA-14 ✅, BUG-NEW-20 ✅, BUG-AUDIT-131 ✅, BUG-AUDIT-144 ✅, BUG-AUDIT-146 ✅, BUG-AUDIT-165-CPP ✅, BUG-AUDIT-166-CPP ✅, BUG-AUDIT-167-CPP ✅, BUG-AUDIT-168-CPP ✅, BUG-AUDIT-169-CPP ✅ | Re-audited 2026-05-13 (C++): 10 unchecked items resolved — 8 PASS, 1 N/A (ASET cross-isolation), 1 FIXED (BUG-AUDIT-166-CPP EL2_E2H entryVmid not zeroed — smmu.cpp:809-811 added; test EL2E2HStreamHasNoVmidTag). All §3.17/§3.17.1/§3.17.5/§3.17.6 checkboxes marked [x]. |
...
| Section | Title | C++ | Rust | Bugs | Notes |
|---------|-------|-----|------|------|-------|
| §3.18 | Interrupts and notifications | ✅ | ✅ | BUG-AUDIT-147 ✅ | GERROR/GERRORN toggle protocol correct; MSI/wired simulated (no real HW writes); interrupt notification ordering satisfied structurally. C++ verified 2026-04-12; TDD: `P3IrqCtrl.IrqCtrlAckMirrorsWrite`. Re-audited 2026-05-04: 11 items checked (4 PASS, 4 N/A, 3 N/A MSI), 1 bug fixed (BUG-AUDIT-147). Re-audited 2026-05-10 (C++ only): 10 items checked (5 PASS, 5 N/A), 0 bugs — IDR0.MSI=0 consistently advertised; ordering requirements vacuously satisfied (no interrupt emission mechanism); wired-only model confirmed coherent. |
| §3.18.1 | MSI synchronization | N/A | ✅ | | CMD_SYNC ensures MSI completion; SW model has no actual MSI writes; ordering satisfied structurally. Re-audited 2026-05-10: 3 items all N/A — IRQ_CTRL disable is synchronous echo; CMD_SYNC serializes under queueMutex; MSI_*_ABT_ERR bits defined in types.h but never set (correct for IDR0.MSI=0). |
| §3.18.2 | Interrupt sources | ✅ | ✅ | BUG-QA-7 ✅, AUDIT-55 ✅, AUDIT-65 ✅, BUG-AUDIT-147 ✅ | SEV gated on IDR0.SEV; CS=0b11 → CERROR_ILL; CS=0b10 with SEV=0 → SIG_NONE (deliberate safe fallback per AUDIT-65). C++ `setIrqCtrl()`/`getIrqCtrlAck()` conformant (smmu.cpp:3568-3577). BUG-AUDIT-147: eventq_irq_pending set on empty→non-empty transition when EVENTQ_IRQEN=1; priq_irq_pending same pattern for PRI queue; 5 TDD tests in test_bug_audit147_eventq_priq_irq.rs all pass. |
| §3.19 | Power control | ✅ | ✅ | AUDIT-53 ✅, BUG-DORMANT-CPP ✅, BUG-DORMANT-2-CPP ✅ | DORMHINT=1 in IDR0; STATUSR.DORMANT set after shutdown()/disable(); power-off preconditions are SW responsibility per spec. C++ BUG-DORMANT-CPP: `disable()` sets `statusr_=1`, `enable()` clears to 0. BUG-DORMANT-2-CPP (2026-05-11): `disable()` now calls `tlbCache->invalidateAll()` before setting DORMANT=1, ensuring §3.19.1 no-cached-translation guarantee. TDD: `P3Dormant.*` in `test_cpp_audit_p3_spotcheck.cpp` (4 tests). |
| §3.19.1 | Dormant state | ✅ | ✅ | BUG-DORMANT-2-CPP ✅ | §3.19.1 no-cache guarantee: `disable()` flushes TLB before setting DORMANT=1. Stalled-transaction abort on dormancy is SW precondition per §3.19.1; power-off preconditions are SW caller obligations. |

### §3.20–3.21 TLB Conflicts and Structure Update Procedures

| Section | Title | C++ | Rust | Bugs | Notes |
|---------|-------|-----|------|------|-------|
| §3.20 | TLB and configuration cache conflict | ✅ | 🚫 | | C++ audit 2026-05-11: 3 PASS, 4 N/A, 0 bugs. Detection IMPL DEF (not implemented). PA access restriction (line 3835) PASS by CacheKey structural keying. Security/VMID/StreamWorld isolation PASS by construction. Rust: IMPL DEF — out of scope |
| §3.20.1 | TLB conflict | ✅ | 🚫 | | C++ audit 2026-05-11: F_TLB_CONFLICT detection N/A (IMPL DEF, sw model uses deterministic hash map). PA access restriction PASS. Cross-VMID/Security/StreamWorld/PA isolation PASS. Cross-stream termination N/A (no conflict ever raised). Rust: IMPL DEF |
| §3.20.2 | Configuration cache conflicts | ✅ | 🚫 | | C++ audit 2026-05-11: F_CFG_CONFLICT detection N/A (IMPL DEF, no actual config cache). Unpredictable-if-not-detected N/A (no config cache). Security state isolation PASS (per-stream StreamContext). Rust: IMPL DEF |
...
| §3.25 | Granule Protection Checks | N/A | N/A | | IDR0.RME_IMPL=0; all 20 items N/A (audited 2026-05-04); SMMU_ROOT_CR0/GPF_FAR/GPT_CFG_FAR not implemented |
| §3.25.1–6 | GPC sub-sections | N/A | N/A | | All sub-sections N/A; GPC/GPT not modeled; no RME support |
...
| Section | Title | C++ | Rust | Bugs | Notes |
|---------|-------|-----|------|------|-------|
| §4.1.1 | Command opcodes | ✅ | ⚠️ | AUDIT-72, BUG-AUDIT-132 | C++ 2026-05-08: all 14 known opcodes present + dispatched; reserved → CERROR_ILL default:; IMPDEF 0x80-0x8F N/A (software model raises CERROR_ILL — within spec latitude) |
| §4.1.2 | Submitting commands to the Command queue | ✅ | ⚠️ | | C++ 2026-05-08: CMDQEN=0 → early return (smmu.cpp:3943); empty-queue exit correct; GERROR.CMDQ_ERR blocks processing (smmu.cpp:3987); SEV/WFE N/A (software model) |
| §4.1.3 | Command errors | ✅ | ⚠️ | AUDIT-72, 93, BUG-AUDIT-132, BUG-AUDIT-157-CPP | C++ 2026-05-08: CERROR_ILL raised for reserved opcode + invalid params; CMDQ_CONS.ERR written via writeCmdqConsErr(); CONS.RD not advanced on error; CERROR_ABT N/A; BUG-AUDIT-157-CPP (CERROR_ATC_INV_SYNC) pre-existing open |
| §4.1.4 | Consumption of commands from the Command queue | ✅ | ⚠️ | AUDIT-93, BUG-CPP-1/RUST-1 | C++ 2026-05-08: pop_front()+CONS.RD advance only on success path (smmu.cpp:4021,4039); CONS.ERR bits preserved across advance; CMD_SYNC completion semantics correct |
| §4.1.5 | Reserved fields | ✅ | ⚠️ | | C++ 2026-05-08: implementation elects to detect SSec RES0 violations and raise CERROR_ILL where defined; all non-specified fields silently ignored otherwise — both options are spec-permitted per §4.1.5 |
| §4.1.6 | Common command fields | ✅ | ⚠️ | BUG-NEW-28/32, BUG-NEW-15/16/17/21, BUG-NEW-24/25, BUG-NEW-37/38 | C++ 2026-05-08: SSec=1 on NS queue → CERROR_ILL for all commands that define SSec field; VA[11:0] taken as zero via TLB page-alignment; PA/IPA field layout correct |
| §4.1.7 | Out-of-range parameters | ✅ | ⚠️ | AUDIT-72 | C++ 2026-05-08: out-of-range SID/SSID/ASID/VMID all produce no-op or fault — satisfies CONSTRAINED UNPREDICTABLE "no effect" option; conformant |

### §4.2 Prefetch Commands

| Section | Title | C++ | Rust | Bugs | Notes |
|---------|-------|-----|------|------|-------|
| §4.2.1 | CMD_PREFETCH_CONFIG | ✅ | ⚠️ | BUG-NEW-25 | C++ 2026-05-11: SSec guard correct (CERROR_ILL on NS queue); out-of-range address IGNORED (no-op; spec doesn't require check); SSV+SSIDSIZE=0/S1CDMax=0 CONSTRAINED UNPREDICTABLE → no-op conformant; SMMUEN=0 consumed not prefetched (no-op IS no-prefetch); HTTU AF/NS/Stride all N/A (SW model; SMMUv3.2; NS queue only) — all 9 audit items PASS or N/A; 0 new bugs |
| §4.2.2 | CMD_PREFETCH_ADDR | ✅ | ⚠️ | BUG-NEW-25 | C++ 2026-05-11: same SSec guard; out-of-range address silently IGNORED; NS param RES0 on NS queue (no Secure queue); Stride N/A (SMMUv3.2, CommandEntry has no stride field); HTTU speculative AF update N/A (SW model has no page table writes) — all audit items PASS or N/A; 0 new bugs |
...
| Ch. 4 Commands | 52 | 5 (3 N/A + 2 ✅) | 45 | 0 | 2 |
...
**Last updated**: 2026-05-13 (C++ §3.17 conformance re-audit: 10 unchecked items resolved — 8 PASS, 1 N/A (ASET §3.17.1 line 3512), 1 FIXED (BUG-AUDIT-166-CPP EL2_E2H entryVmid not zeroed; smmu.cpp:809-811 added; TDD: EL2E2HStreamHasNoVmidTag). All §3.17/§3.17.1/§3.17.5/§3.17.6 checkboxes marked [x]; 206/207 C++ tests pass (pre-existing failure: test_asid_vmid_tlb_spec.VMIDTargetedInvalidation_TLBI_S2_IPA). Previously C++ §3.10.3 conformance re-audit: 4 items checked, 0 PASS, 4 N/A, 0 bugs — all requirements gated on IDR0.RME_IMPL=1 and SMMU_R_* Realm programming interface; model advertises RME_IMPL=0 (smmu.cpp:5354); BUG-AUDIT-161/162/163-CPP re-triaged N/A; §3.10.3/3.10.3.1/3.10.3.3 C++ status upgraded ⚠️→✅. Previously C++ §3.10.1 conformance re-audit: 3 items checked, 1 PASS, 2 N/A, 0 bugs — SECURE_IMPL==0 default NS PASS; SECURE_IMPL==1 branch N/A (conditional on capability not advertised; NS-only model); SEC_SID 2-bit RME encoding PASS (types.h:508-517). BUG-AUDIT-159-CPP confirmed N/A; §3.10.1 C++ status upgraded ⚠️→✅. TASKS_OPERATION_CPP.md §3.10.1 checkbox [x] marked. Previously C++ §3.10.2 conformance re-audit: 4 items checked, 0 PASS, 4 N/A, 0 bugs — SMMU_S_IDR1/SMMU_S_CR0 not implemented; NS-only model; Secure queue/table N/A (BUG-AUDIT-159-CPP re-triaged N/A); SIF N/A (BUG-AUDIT-160-CPP re-triaged N/A); event queue routing N/A (hardware MMIO ring concern). TASKS_OPERATION_CPP.md §3.10.2/3.10.2.1 checkboxes all marked [x]. Previously C++ §3.9.4 conformance audit: 21 items checked, 0 PASS, 21 N/A, 0 bugs — SMMU_R_IDR3 not implemented (RME_IMPL=0); no PCIe TLP wire-format processing; T/TE/XT fields and Realm Command queue items out of scope; TASKS_OPERATION_CPP.md §3.9.4 checkboxes all marked [x]. Previously 2026-05-11 C++ §4.2 conformance re-audit: 9 items checked, all PASS or N/A, 0 new bugs — §4.2.1 CMD_PREFETCH_CONFIG and §4.2.2 CMD_PREFETCH_ADDR both upgraded to ✅; CURRENT_SECTION advanced to §4.3. Previously C++ §3.20 conformance audit: 7 items checked, 3 PASS [PA access restriction line 3835 — CacheKey structural keying; VMID/Security/StreamWorld/PA isolation line 3837 — securityState in CacheKey + S2PS bounds at smmu.cpp:2797-2798; config-cache Security isolation line 3864 — per-stream StreamContext], 4 N/A [F_TLB_CONFLICT detection IMPL DEF — sw model uses deterministic hash map; cross-stream termination vacuously impossible; F_CFG_CONFLICT detection IMPL DEF — no actual config cache; unpredictable-if-not-detected N/A — no config cache], 0 bugs. TASKS_OPERATION_CPP.md §3.20/3.20.1/3.20.2 checkboxes all marked [x]. Previously C++ §3.19 conformance audit: 3 items checked, 1 PASS [§3.19.1 DORMANT no-cache guarantee via BUG-DORMANT-2-CPP fix: `disable()` now calls `tlbCache->invalidateAll()` before setting DORMANT=1; TDD: `P3Dormant.TlbFlushedOnDormantEntry`], 2 N/A [power-off preconditions are SW caller obligations; wakeup=constructor], 1 bug fixed — BUG-DORMANT-2-CPP. TASKS_CPP_OPERATION.md §3.19/3.19.1 checkboxes all marked [x]. Previously 2026-05-10: C++ §3.18 conformance re-audit: 13 items checked, 5 PASS, 8 N/A, 0 bugs — IDR0.MSI=0 consistently advertised; wired-only model; all MSI-specific items N/A by configuration; ordering requirements vacuously satisfied; TASKS_CPP_OPERATION.md §3.18/§3.18.1 checkboxes all marked [x]. Previously C++ §3.15 conformance audit: 9 items checked, 2 PASS, 6 N/A, 1 bug — BUG-AUDIT-164-CPP FIXED: `oshRequired()` helper `(memAttr <= 0x5u) || (memAttr == 0x8u) || (memAttr == 0xCu)` added to stream_context.cpp and smmu.cpp; three OSH enforcement sites updated to cover all Device sub-types (0x0–0x3, 0x4, 0x8, 0xC) and Normal-iNC-oNC (0x5); test_bug_audit164_osh_memattr.cpp: 13 TDD tests all PASS; test_bug4_dead_code_lookup.cpp memAttr updated 0x5→0x77 (Normal WB) for spec-correct TLB plumbing test; 201/202 tests pass (test_bug3_stall_pending_fields pre-existing). All §3.15 TASKS_CPP_OPERATION.md checkboxes marked [x]. Previously C++ §3.14 conformance audit: 3 items checked, 0 PASS, 3 N/A, 0 bugs — speculative marking is IMPLEMENTATION DEFINED; TransactionType enum has no speculative variant and translate() API has no isSpeculative parameter; conformant by omission. All §3.14 TASKS_CPP_OPERATION.md checkboxes marked [x]. Previously C++ §4.1 conformance audit: 28 items checked, 22 PASS, 6 N/A, 0 bugs — all §4.1.1–4.1.7 items PASS; SEV/WFE and CERROR_ABT N/A for software model; IMPDEF opcodes 0x80-0x8F N/A; BUG-AUDIT-157-CPP (CERROR_ATC_INV_SYNC) pre-existing open from §3.9. Previously C++ §3.10 conformance audit: 25 items checked, 11 PASS, 9 N/A, 5 bugs — BUG-AUDIT-159-CPP: no separate Secure command/event queues or Secure stream table [HIGH]; BUG-AUDIT-160-CPP: SMMU_S_CR0.SIF behavior not implemented [MEDIUM]; BUG-AUDIT-161-CPP: CD.NSCFG0/1 not ignored for Realm streams [LOW]; BUG-AUDIT-162-CPP: no Realm Command queue; Realm SSec=1 CERROR_ILL and Realm default NS attribute missing [MEDIUM]; BUG-AUDIT-163-CPP: STE.NSCFG output PA derivation inert — nsCfgOut never consumed [HIGH]. All §3.10 TASKS_CPP_OPERATION.md checkboxes updated. Previously 2026-05-07: C++ §3.5 conformance audit: 43 items checked, 22 PASS, 20 N/A, 1 bug — BUG-AUDIT-154-CPP FIXED: `processEventQueue()` now drains `stallPending_` into `eventQueue` after consuming entries per §3.5.3 line 1824; TDD: `test_bug_audit154_stall_drain.cpp`; 200/200 C++ tests pass. Previously 2026-05-05: BUG-AUDIT-148-CPP FIXED: `submitPageRequest()` silent-drop guard + `processPRIQueue()` no-op guard + `setPRISupported(false)` queue/index clear; 4 TDD tests in `test_bug_audit148_pri_gate.cpp`; 196/196 C++ tests pass. Previously C++ §3.1 conformance audit: 6 items, 3 PASS, 1 PARTIAL, 2 N/A. Previously §3.26 conformance audit: 9 items checked, 2 PASS [IDR3.S1PI=0 → S1PIE/CD.PIE structurally RES0; S1POE absence is spec-mandated], 7 N/A [FEAT_S1PIE/FEAT_S2PIE not implemented; IDR3.S1PI=0 and S2PI=0; S2PTW enforced independently], 0 bugs. All §3.26 TASKS_OPERATION.md checkboxes marked [x]. Previously §3.25 conformance audit: 20 items checked, all N/A — IDR0.RME_IMPL=0; SMMU_ROOT_CR0/GPF_FAR/GPT_CFG_FAR not implemented; GPC/GPT subsystem absent; all §3.25 TASKS_OPERATION.md checkboxes marked [x]. Previously §3.23 conformance audit: 4 items checked, 1 PASS [Tag Unchecked — pure translation engine, no allocation tag writes by construction], 3 N/A [CD.MAIR0/1 field parsing absent — flat model; MTEPERM=1 suppresses a fault path that never exists; 4-bit stage-2 MemAttr decode absent — pre-resolved 8-bit MAIR bytes used throughout]. All §3.23/3.23.1 TASKS_OPERATION.md checkboxes marked [x]. Previously §3.22 conformance audit: 10 items checked, all N/A — AMBA AXI5 DR/RCI/W-DCP/NW-DCP sideband transaction types absent from `TransactionType` enum; STE.DRE/STE.DCP fields absent from `StreamConfig`; output interconnect model absent; entire section inapplicable to software simulation. All §3.22 TASKS_OPERATION.md checkboxes marked [x]. Previously §3.21 conformance re-audit: 34 items checked, 9 PASS [ATOS path, SMMUEN=0 TLB gate, IDR3.BBML=0b01, nT TLB-conflict structural impossibility, command queue write exclusion, stream validity check, invalid-pointer guard, SMMUEN=0 STE gate, CD-from-valid-STE, table bounds enforcement], 25 N/A [hardware pipeline completion semantics, BBM/nT/Contiguous hardware descriptor fields, Level 2 BBML inapplicable, DSB/CFGI programmer procedures, CONSTRAINED UNPREDICTABLE outcomes, speculative prefetch permissions, Shareability-domain ordering, FEAT_LSE2 atomicity], 0 FAIL, 0 new bugs. All §3.21 TASKS_OPERATION.md checkboxes marked [x]. Previously §3.20 conformance audit: 7 items checked, 4 PASS [structural invariants], 2 🚫 [F_TLB_CONFLICT/F_CFG_CONFLICT detection — IMPL DEF, not required], 1 N/A [unpredictable clause is permissive], 0 bugs. TASKS_OPERATION.md §3.20/3.20.1/3.20.2 checkboxes all marked [x]. Previously §3.19 conformance re-audit: 3 items checked, 1 PASS [STATUSR.DORMANT via shutdown()], 2 N/A [power-off preconditions are caller obligations; wakeup=SMMU::new()], 0 bugs. TASKS_OPERATION.md §3.19/3.19.1 checkboxes all marked [x]. Previously §3.18 conformance re-audit: 11 items checked, 4 PASS, 7 N/A [MSI/wired model], 1 bug fixed — BUG-AUDIT-147: eventq_irq_pending and priq_irq_pending now set on empty→non-empty transition when EVENTQ_IRQEN/PRIQ_IRQEN=1; 5 TDD tests in test_bug_audit147_eventq_priq_irq.rs all pass. Previously 2026-05-02: §3.10 conformance audit: 6 requirements checked [3 in-scope + 3 N/A for §3.10.1 Secure/Realm], 5 PASS, 1 N/A [Secure streams 🚫], 0 FAIL, 0 new bugs. NS→NS PA enforcement confirmed at address_space/mod.rs:583. SecurityState::Root=0b11 is an internal PA-space tag, not an SEC_SID input — doc clarification only. 10 spec-anchored tests added in test_bug_audit140_sec_sid_spec.rs. All §3.10 and §3.10.1 TASKS_OPERATION.md checkboxes marked [x]. Previously 2026-04-30: §3.7 conformance audit: 2 items both N/A — physical memory-map layout statements, inapplicable to software model, 0 new bugs. All §3.7 TASKS_OPERATION.md checkboxes marked [x]. Previously §3.6 conformance audit: 6 items all N/A — pure "Arm expects" guidance language, no SMMU behavioral requirements, 0 new bugs. All §3.6 TASKS_OPERATION.md checkboxes marked [x]. Previously §3.5 conformance re-audit: 29 requirements checked, 26 PASS, 5 N/A [Secure queues ×2, EVENTQ IRQ ordering, ECMDQ ×14 🚫], 0 FAIL, 0 new bugs. All §3.5.1–3.5.5 TASKS_OPERATION.md checkboxes marked [x]. Previously 2026-04-29: §3.4 conformance re-audit: 40 requirements checked, 12 PASS, 1 FAIL, 7 PARTIAL, 18 N/A. New bug: BUG-AUDIT-138 OPEN — canonical VA sign-extension check absent in Rust `mod.rs:5102-5212`; non-canonical VA with magnitude `< 2^(64-T0SZ)` passes unchecked. BUG-AUDIT-139 resolved N/A (queue/MSI base addresses are VecDeque internal state, not PA emissions — same rationale as S1ContextPtr/STE-fetch modeling gaps). Previously 2026-04-28: §3.3 conformance re-audit: 35 requirements checked, 18 PASS, 4 FAIL [BUG-AUDIT-134/135/136/137], 6 PARTIAL, 3 N/A, 4 🚫 out-of-scope. New bugs: BUG-AUDIT-134 — no `s1p=false AND s2p=false` guard; BUG-AUDIT-135 — no cross-stream ASID/VMID/StreamWorld TLB-config consistency check; BUG-AUDIT-136 — no validation that streams sharing ASID/VMID/StreamWorld use same TTB0/granule; BUG-AUDIT-137 — no STRW-conditional EPD1=1 enforcement for El2/El3 StreamWorlds. Previously [2026-04-22]: Wiki conformance sweep [2026-04-22l]: memory-tagging-extension.md — 7 claims PASS, 0 bugs; mpam.md — 10 claims PASS, 0 bugs; smmu-fault-model.md — 8 claims PASS, 0 bugs; smmu-pcie-ats-integration.md — 6 claims PASS, 0 bugs; smmu-queue-mechanics.md — 7 claims PASS, 0 bugs; smmu-register-map.md — 12 claims PASS, 0 bugs; smmu-version-feature-map.md — 6 claims PASS, 0 bugs; ihi0070g-b-smmuv3-architecture-spec.md — 2 claims PASS, 0 bugs. **WIKI CONFORMANCE SWEEP COMPLETE — all 0 pages remain unaudited.** Previous [2026-04-22k]: external-interfaces.md — 9 claims PASS, 0 bugs; interrupts-and-power.md — 12 claims PASS, 0 bugs; mec.md — 8 claims PASS, 0 bugs. Previous [2026-04-22j]: coherency-and-embedded-implementations.md — 2 bugs fixed [COHACC==1 "Inner Shareable" → "IO-coherent" per spec §6.3.1/§3.15; VMSAv9-128 FEAT_LSE2 requirement removed — FEAT_LSE2 governs config-structure atomicity, not descriptor atomicity per §3.15/§3.21.3]; debug-trace.md — 3 claims PASS, 0 bugs; destructive-reads.md — 8 claims PASS, 0 bugs. Previous [2026-04-22i]: atos.md — 6 claims PASS, 0 bugs; attribute-transformation.md — 4 claims PASS, 0 bugs; command-formats.md — 8 claims PASS, 0 bugs. Previous [2026-04-22h]: pcie-ats-pri.md — 2 bugs fixed [§3.9.4.1–3 T-bit-not-ignored claim; §3.9.4.3 XT table T=0/XT=1 interpretation]; device-permission-table.md — 8 claims PASS, 0 bugs; synthesis/smmu-translation-pipeline.md — 4 claims PASS, 0 bugs. Previous 2026-04-22: event-queue.md, httu.md, granule-protection-check.md — all 24 claims PASS, 0 bugs. Previous: 2026-04-12 C++ re-audit complete: BUG-AUDIT-133-CPP [§5.2 INSTCFG encoding], BUG-13.1.2-CPP-A [N/A — no Rust bug in C++], BUG-13.1.4-CPP-A [§13.1.4 ATOS bypass], BUG-13.1.5-CPP [§13.1.5 two-stage MemAttr], BUG-13.2-CPP [§13.2 GBPA Device→OSH], BUG-7.3.1-CPP [§7.3.1 MEV stall guard], BUG-GERROR-DPT-CPP [§7.5 GERROR_DPT_ERR], BUG-SEC8-SECURE-PRI-CPP [§8.2 Secure stream discard], BUG-AUDIT-130-CPP [§3.13.4 stage-2 HTTU update], BUG-DORMANT-CPP [§3.19 STATUSR.DORMANT], BUG-13.1.7-CPP [§13.1.7 Device/NC→OSH in applyOutputAttrs+TLB fast path] — all fixed ✅.)
**Current bug count**: BUG-AUDIT-01 through BUG-AUDIT-138 — all fixed ✅; BUG-AUDIT-144/146/147 — all fixed ✅; BUG-AUDIT-148-CPP — FIXED ✅; BUG-AUDIT-149-CPP/152-CPP/153-CPP — FIXED ✅ (§3.3/§3.4 C++ audit 2026-05-06); BUG-AUDIT-154-CPP — FIXED ✅ (§3.5 C++ audit 2026-05-07: `processEventQueue()` stallPending_ drain); BUG-AUDIT-155-CPP/156-CPP/158-CPP — FIXED ✅ (§3.9 ATS audit 2026-05-08); BUG-AUDIT-157-CPP — OPEN (CERROR_ATC_INV_SYNC infrastructure pending); BUG-AUDIT-159-CPP/160-CPP — N/A (re-triaged 2026-05-13: Secure programming interface out of scope for NS-only model); BUG-AUDIT-161-CPP/162-CPP/163-CPP — N/A (re-triaged 2026-05-13: Realm programming interface out of scope; IDR0.RME_IMPL=0; no SMMU_R_* registers); BUG-AUDIT-164-CPP — FIXED ✅ (§3.15 C++ audit 2026-05-09: oshRequired() helper covers all Device+NC types); BUG-AUDIT-165-CPP — FIXED ✅ (§3.17 re-audit 2026-05-13: globalTranslations→nonGlobal, Secure-from-NS override, global ASID-eviction); BUG-AUDIT-166-CPP — FIXED ✅ (§3.17 re-audit 2026-05-13: EL2/EL3 entryVmid zero; EL2_E2H entryVmid zero added smmu.cpp:809-811); BUG-AUDIT-167-CPP — FIXED ✅ (§3.17 re-audit 2026-05-13: S2P==0 broadcast VMID=0 substitution); BUG-AUDIT-168-CPP — FIXED ✅ (§3.17.5 re-audit 2026-05-13: CR2.E2H change flushes EL2/EL2_E2H TLBs); BUG-AUDIT-169-CPP — FIXED ✅ (§3.17.6 re-audit 2026-05-13: VMW wildcard applied to all NH TLBI commands); BUG-12.3-A, BUG-13.1.2-A/B, BUG-13.1.4-A/D/CPP-A, BUG-13.1.5-A/CPP, BUG-13.1.7-CPP, BUG-13.2-A/CPP — all fixed ✅; BUG-7.3.1-CPP, BUG-GERROR-DPT-CPP, BUG-SEC8-SECURE-PRI-CPP, BUG-AUDIT-130-CPP/133-CPP, BUG-DORMANT-CPP, BUG-DORMANT-2-CPP — all fixed ✅
**Additional named batches fixed**: CONF-GAP series, BUG-QA series, BUG-NEW series, BUG-CPP/RUST series
**Test status**: C++ 202/207 (pre-existing failure: BUG-AUDIT-54 PTM test) | Rust 3707/3707 (all suites green) | 0 clippy warnings
