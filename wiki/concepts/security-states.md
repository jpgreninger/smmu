...
- Realm state requires Granule Protection Checks (GPT lookups) on all transaction PA outputs — see [[concepts/granule-protection-check]].
...
- [[concepts/granule-protection-check]] — GPC checks applied to Realm-state transactions
- [[concepts/two-stage-translation]] — each security state has independent translation pipeline
- [[concepts/stream-table-entry]] — Secure and Realm STEs are in separate stream tables
- [[concepts/command-queue]] — separate Command queues per security state
- [[concepts/event-queue]] — events routed to the queue matching the stream's security state
- [[concepts/smmu-initialization]] — separate enable sequences per security state
- [[concepts/pcie-ats-pri]] — T/TE/XT bits in PCIe map to Realm security state

## Sources That Use This Concept

- [[sources/ihi0070g-b-smmuv3-architecture-spec]] — §3.10 Security states support; §3.10.1 StreamID Security state (SEC_SID); §3.10.2 Support for Secure state; §3.10.2.2 Secure EL2; §3.10.3 Support for Realm state; §2.6 SMMU for RME features; §2.7 SMMU for RME DA features
