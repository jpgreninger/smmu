...
All Realm stream transactions (and some Non-secure with `RME_IMPL`) are subject to GPC at the PA output. See [[synthesis/smmu-translation-pipeline]] Step 10 and [[concepts/granule-protection-check]].

### DPT (RME DA)

Realm streams with ATS and `STE.EATS == 0b11` are subject to DPT checks. See [[concepts/device-permission-table]].
...
Each state requires its own initialization sequence (see [[concepts/smmu-initialization]]):
...
- [[concepts/security-states]] — concept-level description
- [[concepts/granule-protection-check]] — GPC for Realm
- [[concepts/device-permission-table]] — DPT for Realm ATS
- [[concepts/pcie-ats-pri]] — PCIe T/XT/TE bits and Realm state mapping
- [[synthesis/smmu-translation-pipeline]] — pipeline with security state gates
